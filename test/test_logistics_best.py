...
def main():
    ...
    # 加载最佳模型
    import os
    model_dir = os.path.join(project_root, "models", "multi_agent_logistics", "mappo", "best")
    ...
    for agent_id, agent in trainer.agents.items():
        ...
        if os.path.exists(model_path):
            try:
                ...
                print(f"✗ 加载最佳模型失败 {model_path}: {str(e)}")
        else:
            print(f"⚠ 最佳模型不存在: {model_path}，尝试加载最新模型")
            model_path = os.path.join(project_root, "models", "multi_agent_logistics", "mappo", f"{agent_id}_agent.pth")
            if os.path.exists(model_path):
                try:
                    agent.load(model_path)
                    print(f"✓ 已加载最新模型: {model_path}")
                    loaded_count += 1
                except Exception as e:
                    print(f"✗ 加载最新模型失败 {model_path}: {str(e)}")
    
    if loaded_count == 0:
        print("警告: 没有加载到任何模型")
        return
    
    print(f"\n开始测试最佳模型... ({env.max_steps} 步)")
    print("="*80)
