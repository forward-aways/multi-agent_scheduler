...
def main():
    ...
    # 注意：手动模式下不自动生成订单，所以需要在训练循环中保持手动订单
    # 但环境代码期望pending_orders是列表格式，all_orders是字典格式
    # 由于格式不一致，手动模式测试比较复杂，这里直接使用随机订单模式
